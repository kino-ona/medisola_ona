...
var CustomDietSummary = (function() {
  ...
  function renderSummary() {
    ...
    if (!state.readOnly) {
      html += '<button class="sp-right-edit" onclick="history.back()">수정 <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></button>';
    }
    html += '</div>';
  ...
  return {
    init: function(config) {
      ...
      if (!restoreState()) {
        console.warn('[CustomDietSummary] No saved state. readOnly=' + config.readOnly + ', selections=', config.selections);
        if (!config.readOnly) history.back();
        return;
      }

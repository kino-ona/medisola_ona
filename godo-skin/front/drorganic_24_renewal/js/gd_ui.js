    ...
    $('.top_search_cont input[name="keyword"]').on({
        ...
        blur: function () {
            $("body").click(function (e) {
                if (
                    !$(".search_cont").has(e.target).length &&
...
$(document).ready(function () {
    ...
    $(
        "#quick_search .btn_quick_search_open, #quick_search .btn_quick_search_close"
    ).on({
        click: function () {
            gd_quick_motion();
        },
...
function gd_center_layer() {
    ...
    $(document).on(
        "click",
        ".layer_wrap .layer_close, .btn_box .btn_cancel",
        function () {
...
function gd_close_layer() {
    if ($(".layer_wrap").is(":visible") || $("#layerDim").is(":visible")) {
        if ($(".layer_wrap .layer_close, .btn_box .btn_cancel").length > 0) {
            $(".layer_wrap .layer_close, .btn_box .btn_cancel").trigger(
                "click"
            );
        } else {
            // 딤만 떠있는 경우
...
function gd_carttab_layer() {
    ...
    $(
        ".btn_shop_cart_box .btn_shop_cart_open, .btn_shop_cart_box .btn_shop_cart_close"
    ).on({
        click: function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data == "false") {
                ...
                var addHtml =
                    '<div class="gnb_allmenu" style="display:none;"><div class="gnb_allmenu_box">';
                addHtml += "<ul>";
                ...
                addHtml += "</ul>";
                addHtml +=
                    '</div><span class="btn_all_menu_close" onClick="gd_btn_all_menu_close();">전체메뉴닫기</span></div>';

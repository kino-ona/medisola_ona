...
wm.layer = {
    popup: function (src, w, h, scrolling) {
        ...
        if ($obj1.length == 0) {
            var html = "<div id='layer_popup' style='border-radius: 20px;'>";
...
wm.card = {
    register: function (w, h) {
        ...
        if (path == '/subscription/card_list.php') {
            const url = "/subscription/card_register.php?isMypage=1";
        } else {
            const url = "/subscription/card_register.php";
        }
    ...
    $(".js_add_card_order_btn").click(function () {
        ...
        $('form').find(':input').each(function () {
            ...
            if (type === 'radio' || type === 'checkbox') {
                if ($(this).is(':checked')) {
                    storageData[name] = $(this).attr('id');
                }
            } else {
                storageData[name] = $(this).val();
            }
        });

        sessionStorage.setItem(sessionKey, JSON.stringify(storageData));

var gd_goods_view = function () {
    ...
    this.input_count_change = function (inputName, goodsFl) {
        ...
        if (
            ...
            alert(
                __(
                    "쿠폰이 적용된 상품은 수량변경을 할 수 없습니다.%1$s쿠폰취소 후 수량을 변경해주세요.",
                    "\n"
                )
            );
    ...
    this.count_change = function (
        ...
        var firstHiddenAddPrice = $("li.hidden_add_price").first();
                ...
                if (
                    ...
                    if (isComponentGoodsCountChange && componentGoodsCntSum >= componentMaxSelection) {
                        ...
                        if (addedPriceOptionExist) {
                            ...
                            const firstOption = firstHiddenAddPrice.find('ul:first li:first');
                            const addedPriceValue = firstOption.data("params").split(setStrDivision)[0]?.split(setIntDivision)[1] || 1000;
        ...
        if (isComponentGoodsCountChange && addedPriceOptionExist) {
            ...
            const firstOption = firstHiddenAddPrice.find('ul:first li:first');
            var addedPrice = firstOption.data("params").split(setStrDivision)[0]?.split(setIntDivision)[1];
            ...
            setControllerName.add_goods_select(firstOption, totalOverflowedCount, true);
        }
    ...
    this.option_price_display = function (inputName) {
        ...
        if (setOptNo != "" && setOptNo != undefined) {
            if ($(inputName).data("params") == undefined) {
                ...
                if ($(".js_option_select" + setOptNo).length == 1)
                    $(".js_option_select" + setOptNo).html(
                        __("옵션을 선택하세요.")
                    );
            }
        }
        ...
        if (setOptionTextFl == "y") {
            if (!this.option_text_valid(frmId)) {
                if (setOptNo == "" || setOptNo == undefined) {
                    ...
                    alert(
                        __(
                            "선택한 옵션의 필수 텍스트 옵션 내용을 먼저 입력해주세요."
                        )
                    );
        ...
        if (setOptionDisplayFl == "s") {
            ...
            $(".js_multi_option .js_option_select").html(
                __("옵션을 선택하세요.")
            );
            $(".js_multi_option .js_option_select:not(:first)").addClass(
                "disabled"
            );
        }
        ...
        if (setMileageUseFl == "y" && arrTmp[2]) {
            $(frmId + ' input[name="set_goods_mileage"]').val(
                parseFloat(arrTmp[2].trim())
            );
        }

        if (arrTmp[3])
            $(frmId + ' input[name="set_goods_stock"]').val(
        ...
        if (
            $(frmId + " div.optionKey_" + arrTmp[0]).length &&
            setOptionTextFl != "y"
        ) {
            ...
            if (setOptionPriceFl == "y" && optionPrice) {
                ...
                var optionDisplayTextPrice =
                    ...
                    ")";
                $(
                    "[id^=option_display_item_" + displayOptionkey + "] dl > dt"
                ).append(optionDisplayTextPrice);
            }

            $(
                frmId +
                    " div#option_display_item_" +
                    displayOptionkey +
                    " div.optionKey_" +
                    arrTmp[0] +
                    "  .goods_cnt"
            ...
            $(
                frmId + " div.optionKey_" + arrTmp[0] + "  button.delete_goods"
            ).on("click", function (e) {
    ...
    this.remove_option = function (optionId) {
        ...
        var optionCnt =
            $("div[id*='option_display_item_']").length +
            $("[id^='component_option_display_item_']").length;
        if (optionCnt == 0) $(frmId + " div.option_total_display_area").hide();

        // 옵션을 여러개 생성 후 가운데 옵션을 삭제 했을 때 배열 순서 재정의
        if (setOptionTextFl == "y") {
            $(frmId + " div[id*='option_display_item_" + setOptNo + "']").each(
                function (key, div) {
                    $(div)
    ...
    this.remove_component_option = function () {
        ...
        $("[id^='option_display_item_']").remove();
    ...
    this.default_component_options = function () {
        ...
        const optionText = $(".js_option_select").html();

        const match = optionText.match(/(\d+)식/);
        let allowedCount = 0;
        if (match) {
        ...
        const weeksMatch = optionText.match(/(\d+)주/);
        const totalWeeks = weeksMatch ? parseInt(weeksMatch[1], 10) : 1;
        const allowedMenuSelectableCount = Math.floor(allowedCount / totalWeeks) || 10;
    ...
    this.option_text_select = function (inputName) {
        ...
        var displayOptionDisplay = $(
            frmId + " div[id*='option_display_item_" + setOptNo + "']"
        )
            .last()
            .attr("id");
        ...
        if (displayOptionDisplay) {
            if (setOptNo != "" && setOptNo != undefined) {
                if (
                    $(frmId + " #" + displayOptionDisplay + " div.check")
                        .length == 0
                ) {
                    displayOptionDisplay = displayOptionDisplay + "_0";
                }
            }
            var checkKey = $(frmId + " #" + displayOptionDisplay + " div.check")
                .attr("class")
                .replace("check", "")
                .trim();
            ...
            //if(setOptionFl =='y') {
            var optionItemNo =
                $(frmId + " div[id*='option_display_item_']").length - 1;
            //} else {
            //    var optionItemNo = 0;
            //}
            ...
            if (
                ...
                var emCnt = $(frmId + " div[id*='option_display_item_']")
                    .last()
                    .find("em").length;
            ...
            if (
                $(frmId + " div.optionKey_" + tmpStr[0] + optionTextKey).length
            ) {
                alert("이미 선택된 옵션입니다");
                return false;
            } else {
    ...
    this.component_option_price_display = function (inputName) {
        ...
        if (setOptNo != "" && setOptNo != undefined) {
            if ($(inputName).data("params") == undefined) {
                ...
                $(
                    "[id^=option_display_item_" + setOptNo + "]"
                ).remove();
                if ($(".js_option_select" + setOptNo).length == 1)
                    $(".js_option_select" + setOptNo).html(
                        __("옵션을 선택하세요.")
        ...
        if (setOptNo != "" && setOptNo != undefined) {
            complied = _.template(
                $(
                    "#componentOptionTemplateRelated" + setOptNo + setTemplate
                ).html()
            );
        }
        addHtml += complied({
        ...
        // $(
        //     frmId +
        //         " div#component_option_display_item_" +
        //         displayOptionkey +
        //         " div.optionKey_" +
        //         arrTmp[0] +
        //         "  .goods_cnt"
        ...
        $(frmId + " div.header button.delete_goods").on("click", function (e) {
            $("[id^='component_option_display_item_']").addClass("force_hidden");
            $(".js_option_scroll").css("overflow", "");
        ...
        $(frmId + " div.header button.default_components").on("click", (e) => {
        ...
        $(frmId + " div.header button.empty_components").on("click", (e) => {
        ...
        $(frmId + " div.footer div.completeButton").on("click", function (e) {
            $("[id^='component_option_display_item_']").addClass("force_hidden");
            $(".js_option_scroll").css("overflow", "");
            $(".js_option_scroll").css("overflow-y", "auto");
        ...
        $(".js_component_goods_select").click();
    };

    this.add_component_goods = function (inputName, count, maxSelection) {
        var frmId = "#" + $(inputName).closest("form").attr("id") + ":eq(0)";
        var selAddGoods = $(inputName).data("key");
        ...
        if ($(inputName).data("params") != "") {
            ...
            if (displayOptionDisplay) {
                ...
                if ($(frmId + " #component_goods_display_item_" + displayOptionkey + "_" + displayComponentGoodsKey).length && 
                    (setOptNo == "" || setOptNo == undefined)) {
                    // Already added, exit silently.
                    return false;
                } else {
                    var addHtml = "";
                    var complied = _.template(
                    ...
                    const goodsDescription = $(inputName).next("template").html();
                    ...
                    const _optText = $(".js_option_select").html() || "";
                    const _mealsMatch = _optText.match(/(\d+)식/);
                    const _weeksMatch = _optText.match(/(\d+)주/);
                ...
                // $(frmId + " select[name*='componentGoodsInput']").val(""); //???:
                return false;
            }
        }
    };
    ...
    this.display_selected_component_goods = function () {
        ...
        if (isDefaultStamp) {
            $("[id^='option_display_item_'] .title").html('기본구성으로 선택하였습니다.');
            ...
            const title = "총 " + selectedItems.length + "개 메뉴 " + totalCount + "식을 선택하였습니다.";
    ...
    this.remove_add_goods = function (optionId, addGoodsId) {
        ...
        var addGoodsCnt = $(
            "div[id='option_display_item_" + optionId + "']"
        ).find("div[id*='add_goods_display_item_']").length;
    ...
    this.goods_calculate = function (frmId, goodsFl, itemNo, goodsCnt) {
        ...
        if (goodsFl) {
            ...
            $(frmId + " .option_price_display_" + setOptNo + itemNo).html(
                gd_money_format(
                    (optionTotalPrice + goodsPrice) * goodsCnt
                ).format()
            );
            ...
            if (setOptNo == "" || setOptNo == undefined) {
                ...
                ).val(optionPrice * goodsCnt);
                $(
                    "#option_display_item_" +
                        itemNo +
                        ' input[name="optionTextPriceSum[]"]'
                ).val(optionTextPrice * goodsCnt);
                $(
                    "#option_display_item_" +
                        itemNo +
                        " input[name='goodsPriceSum[]']"
                ).val(goodsPrice * goodsCnt);
            } else {
                $(
                    "#option_display_item_" +
                        setOptNo +
    ...
    this.resetAddedPrice = function () {
        ...
        if (firstHiddenAddPrice.length) {
            const firstOption = firstHiddenAddPrice.find('ul:first li:first');
            setControllerName.add_goods_select(firstOption, 0, true);
        }
    }
    ...
    this.generatePriceAddedGoodsName = function () {
        ...
        $("input[name='componentGoodsAddedPrice[0][]']").each(function () {
            ...
            if (addedPrice > 0) {
                ...
                var baseAddedPrice = $("li.hidden_add_price:first ul:first li:first").data("params")?.split(setStrDivision)[0]?.split(setIntDivision)[1] || 1000;

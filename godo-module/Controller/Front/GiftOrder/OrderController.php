...
class OrderController extends \Controller\Front\Controller
{
	public function pre()
	{
		if (!MemberUtil::checkLogin()) {
			$url = "../member/login.php?guestOrder=1";
			return $this->js("window.location.href='{$url}';");
		}

		$idxRequest = Request::get()->get("idx_request");
		if ($idxRequest) {
			$giftOrder = App::load(\Component\GiftOrder\GiftOrder::class);
			$info = $giftOrder->getRequestInfo($idxRequest);
			$this->setData("requestInfo", $info);
			$this->setData("idxGiftRequest", $idxRequest);
		}

		/* 증정품 처리 S */
		$cart = new Cart();
		$addonGoods = App::load(\Component\AddonGoods\AddonGoods::class);
		$cartSno = [];
		if (Request::get()->has('cartIdx')) {
			$cartSno = $cart->getOrderSelect(Request::get()->get('cartIdx'));
        }

		$addonGoods->apply($cartSno, 'gift');
		/* 증정품 처리 E */
	}
        ...
        try {
            ...
            if (!$cart->orderPossible) {
                if(trim($cart->orderPossibleMessage) !== ''){
                    throw new AlertRedirectException(__($cart->orderPossibleMessage), null, null, '../gift_order/cart.php');
                }
                else {
                    throw new AlertRedirectException(__('구매 불가 상품이 포함되어 있으니 장바구니에서 확인 후 다시 주문해주세요.'), null, null, '../gift_order/cart.php');
                }
            }
            ...
            if (empty($cart->payLimit) === false && in_array('false', $cart->payLimit)) {
                throw new AlertRedirectException(__('결제 가능한 결제수단이 없으니 장바구니에서 확인 후 다시 주문해주세요.'), null, null, '../gift_order/cart.php');
            }

            // 설정 변경등으로 쿠폰 할인가등이 변경된경우
            if (!$cart->changePrice) {
                throw new AlertRedirectException(__('할인/적립 금액이 변경되었습니다. 상품 결제 금액을 확인해 주세요!'), null, null, '../gift_order/cart.php');
            }
                                ...
                                if (empty($_cartInfo['memberCouponNo']) === false) {
                                    ...
                                    foreach ($tmpApplyMemberCouponList as $tmpApplyMemberCouponNo) {
                                        if (array_key_exists($tmpApplyMemberCouponNo, $checkDuplMemberCouponNo) === true) {
                                            throw new AlertRedirectException(__('이미 사용중인 쿠폰이 적용되어 있습니다.'), null, null, '../gift_order/cart.php');
                                        }
            ...
            if ($e->ectName == 'ERROR_VIEW') {
                ...
                throw new AlertRedirectException(__('안내') . $item, null, null, '../gift_order/cart.php', 'parent');

            } else {
                $e->actLog();
                // echo ($e->ectMessage);
                throw new AlertRedirectException(__('오류') . ' - ' . __('오류가 발생 하였습니다.'), null, null, '../gift_order/cart.php', 'parent');
            }
        }
    }
	...
	{
        $this->addCss(["order/order.css", "gift_order/giftorder.css"]);
        $this->addScript(["gift_order/select_card.js"]);
	}
}
...
class KakaoLoginController extends \Bundle\Controller\Front\Member\Kakao\KakaoLoginController
{
    public function index()
    {
        ...
        try {
            ...
            if (empty($utmCampaign) && $returnURLFromAuth) {
                // returnURLFromAuth에서 utm_campaign 파라미터 추출
                if (strpos($returnURLFromAuth, 'utm_campaign=') !== false) {
                    $queryParams = parse_str(parse_url($returnURLFromAuth, PHP_URL_QUERY), $queryParams);
            ...
            if($code = $request->get()->get('code')){
                ...
                // 디버깅용
                // if ($this->get_client_ip()=="183.98.27.142") {
                //     // var_dump($userInfo);

                //     // 사용자가 선택한 약관 동의을 불러오기 위한 API 요청 URL
                //     $returnUrl = 'https://kapi.kakao.com/v2/user/service_terms';
                ...
                //     $allowedServiceTerms= json_decode($userResponse)->service_terms; 

                //     $privateApprovalOption1 = "n";

                //     foreach ($allowedServiceTerms as $key => $value) {
                //         if ($value->tag == "privacy02" && $value->agreed) {
                //             $privateApprovalOption1 = "y";
                //         }
                //         // 마케팅 수신동의 (SMS, 이메일)
                //         if ($value->tag == "sms2" && $value->agreed) {
                //             $request->post()->set('smsFl', "y");
                //         }
                //         if ($value->tag == "email" && $value->agreed) {
                //             $request->post()->set('maillingFl', "y");
                //         }
                //         // 평생회원
                //         if ($value->tag == "lifetime" && $value->agreed) {
                //             $request->post()->set('expirationFl', '999');
                //         }
                //     }
                ...
                if($memberSnsService->validateMemberSns($memberSns)) {
                    ...
                    if($session->has(Member::SESSION_MEMBER_LOGIN)){
                        ...
                        if($kakaoType == 'my_page_password'){
                            ...
                            ";
                            $this->js($js);
                            return; // my_page_password 타입은 여기서 종료
                        }
                        ...
                        if($kakaoType == 'disconnect') {
                            ...
                            return; // disconnect 타입은 여기서 종료
                        }
                        
                        // 이미 로그인된 상태에서 같은 카카오 계정으로 다시 로그인하는 경우
                        // (일반 로그인, join_method 등) - 세션 갱신 및 쿠키 저장을 위해 계속 진행
                        // 이 경우 아래의 loginBySns()와 쿠키 저장 로직이 실행됨
                    }
                    ...
                    // 카카오 아이디 로그인
                    $kakaoUuid = $userInfo['id'];
                    $memberSnsService->saveToken($kakaoUuid, $properties['access_token'], $properties['refresh_token']);
                    $memberSnsService->loginBySns($kakaoUuid);
                    if ($saveAutoLogin == 'y') $session->set(Member::SESSION_MYAPP_SNS_AUTO_LOGIN, 'y');
                    
                    // 자동 로그인 쿠키 저장 (SNS 로그인은 UUID 사용)
                    if ($saveAutoLogin == 'y') {
                        $memberSession = $session->get(Member::SESSION_MEMBER_LOGIN);
                        
                        // SNS 로그인은 비밀번호가 없으므로 UUID를 쿠키에 저장
                        if ($memberSession && isset($memberSession['memId'])) {
                            // SNS 자동 로그인을 위한 별도 쿠키 저장 (UUID 기반)
                            $snsCookieData = [
                                'type' => 'sns',
                                'snsType' => isset($memberSession['snsTypeFl']) ? $memberSession['snsTypeFl'] : 'kakao',
                                'uuid' => \Encryptor::encrypt($kakaoUuid),
                                'memId' => \Encryptor::encrypt($memberSession['memId']),
                            ];
                            
                            // NOTE:
                            // - 브라우저 종료 후에도 남아야 하는 "자동로그인" 쿠키이므로 만료시각을 명시(절대 timestamp)하고,
                            // - 사이트 전역에서 읽히도록 path 를 '/' 로 고정한다.
                            // Cookie::set()은 코드 위치/버전에 따라 상대초/절대시간을 혼용해 쓰는 케이스가 있어
                            // 상대초만 넘기면 환경에 따라 세션 쿠키처럼 처리되거나 의도치 않게 동작할 수 있다.
                            \Cookie::set('GD5ATL_SNS', json_encode($snsCookieData), time() + (3600 * 24 * 20), '/');
                        }
                    }
                ...
                if ($kakaoType == 'join_method') {
                    ...
                    try {
                        ...
                        if ($session->has('wanban')) {  // wanban 세션이 있으면 회원가입 파라미터에 추가
                            $request->post()->set('wanban', $session->get('wanban'));
                        }

                        if ($session->has('utm_campaign')) {  // utm_campaign 세션이 있으면 회원가입 파라미터에 추가
                            $request->post()->set('utmCampaign', $session->get('utm_campaign'));
                        }
                        ...
                        $allowedServiceTerms= json_decode($userResponse)->service_terms; //Access Token만 따로 뺌
                        ...
                        $arrPrivateOfferFl = array("4" => "n", "5" => "n");
                        $arrPrivateConsignFl = array("6" => "n");
            ...
            if ($kakaoType != 'join_method') {
                ...
                if ($startLen = strpos($request->getRequestUri(), "?")) {
                    ...
                    foreach ($requestUriArray as $key => $val) {
                        ...
                        // saveAutoLogin 파라미터 찾기
                        $isSaveAutoLogin = strstr($val, 'saveAutoLogin');
                        if ($isSaveAutoLogin && strpos($val, '=') !== false) {
                            $saveAutoLoginToState = explode('=', $val, 2);
                            if (isset($saveAutoLoginToState[1]) && $saveAutoLoginToState[1] == 'y') {
                                $state['saveAutoLogin'] = 'y';
                            }
                        }
                    }

                    $state['referer'] = $request->getReferer();
                    // URL 파라미터에서 찾지 못한 경우 request 객체에서도 확인
                    if (!isset($state['saveAutoLogin']) && $request->get()->get('saveAutoLogin') == 'y') {
                        $state['saveAutoLogin'] = 'y';
                    }
                    $callbackUri = substr($request->getRequestUri(), 0, $startLen);
                } else {
                    // 쿼리 파라미터가 없는 경우에도 saveAutoLogin 확인
                    if ($request->get()->get('saveAutoLogin') == 'y') {
                        $state['saveAutoLogin'] = 'y';
                    }
                }

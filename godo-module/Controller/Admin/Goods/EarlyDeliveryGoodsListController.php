...
class EarlyDeliveryGoodsListController extends \Controller\Admin\Controller
{
    public function index()
    {
        $this->callMenu('goods', 'goods', 'earlyDeliveryGoods');
        // --- 상품 데이터
        try {

            $in = \Request::request()->all();
            if($in['useEarlyDelivery']){
                $this->setData('useEarlyDelivery', $in['useEarlyDelivery']);
            }else {
                $this->setData('useEarlyDelivery', 'all');
            ...
            if ($getData && $getData['data']) {
                foreach ($getData['data'] as $key => $val) {
                    ...
                    $eData = $earlyDelivery->getEarlyDeliveryData($val['goodsNo']);
                    $getData['data'][$key]['useEarlyDelivery'] = $eData['useEarlyDelivery'];
                    $getData['data'][$key]['earlyDeliveryUrl'] = $eData['earlyDeliveryUrl'];
                }
            }

 ...
 * @author  yjwee
 */
class MemberGroupPsController extends \Bundle\Controller\Admin\Member\MemberGroupPsController
{
        ...
        try {
            ...
            switch (Request::request()->get('mode')) {
                case MemberGroup::MODE_REGISTER:
                    /*
                     * 210929 디자인위브 mh 자체코드 미리 저장 
                     * saveGroup() 갔다오면 바뀜 
                     * selfCode 필수값이 아니라 값이 있으면 중복체크했는지 체크
                     */ 
                    $selfCode = Request::post()->get('selfCode');
                    $chkSelfCode = Request::post()->get('chkSelfCode');
                    
                    if($selfCode && $selfCode != $chkSelfCode){
                        throw new Exception('등급코드 중복확인을 해주세요.');
                    }
                    
                    $groupService->saveGroup();
                    
                    //210929 디자인위브 mh 자체코드 추가
                    $wibMember->memberCodeSave(Request::post()->get('sno'), Request::post()->get('chkSelfCode'), $selfCode);
                ...
                case MemberGroup::MODE_MODIFY:
                    ...
                    //210929 디자인위브 mh 자체코드 추가
                    $wibMember->memberCodeSave(Request::post()->get('sno'), $selfCode);
                ...
                case 'selfCodeCheck':
                    ...
                    $codeCheck = $wibMember->selfCodeCheck(Request::post()->get('sno'), Request::post()->get('selfCode'));
                    
                    if(!$codeCheck){
                        throw new Exception(sprintf(__('%s는 이미 등록된 등급코드입니다'), Request::post()->get('selfCode')));
                    }
                    $this->json(__('사용가능한 등급코드입니다.'));
                    break;
                default:

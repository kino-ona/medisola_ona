...
class MemberDAO extends \Bundle\Component\Member\MemberDAO
{
    ...
    protected function setQuerySearch($params, &$arrBind)
    {
        ...
        if (StringUtils::strIsSet($params['joinedVia']) !== NULL && StringUtils::strIsSet($params['joinedVia']) !== '') {
            $arrWhere[] = 'm.joinedVia = ?';
            $this->db->bind_param_push($arrBind, $this->fields['joinedVia'], StringUtils::strIsSet($params['joinedVia']));
        }
        ...
        $this->db->strField .= ', m.email, m.phone, m.cellPhone, mc.couponCount';
        $this->db->strField .= ', m.zipcode, m.zonecode, m.address, m.addressSub, m.ex6';
        $this->db->strField .= ', m.joinedVia';
        ...
        $this->db->strJoin = ' LEFT JOIN (SELECT memNo, COUNT(memberCouponState) as couponCount from ' . DB_MEMBER_COUPON . ' group by memNo) AS mc ON m.memNo = mc.memNo';

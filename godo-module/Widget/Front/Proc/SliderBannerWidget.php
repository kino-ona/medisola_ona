...
class SliderBannerWidget extends \Bundle\Widget\Front\Proc\SliderBannerWidget
{
  public function index()
  {
    ...
    // 움직이는 배너 정보 세팅
    $setData = [];
    if (strpos($getData['bannerTitle'], '중간') !== false) {
      $setData['bannerPosition'] = 'middle';
    } else {
      $setData['bannerPosition'] = 'top';
    }
    ...
    $setData['brightTextBannerIndices'] = [];
    ...
    foreach ($tmp['bannerInfo'] as $key => $info) {
      ...
      if (empty($info['bannerLink']) === false) {
        ...
        $setData['bannerInfo'][$key]['imageTag'] = '<a href="' . $info['bannerLink'] . '" ' . $tmpTarget . '>' . $tmpImage . '</a>';
      } else {
        $setData['bannerInfo'][$key]['imageTag'] = $tmpImage;
      }

      $setData['bannerInfo'][$key]['image'] = $getData['bannerImagePath'] . $tmp['bannerFolder'] . '/' . $info['bannerImage'];
      $setData['bannerInfo'][$key]['imageAlt'] = $info['bannerImageAlt'];
      if (substr($info['bannerLink'], -2) === '()') {
        $setData['bannerInfo'][$key]['onClick'] = $info['bannerLink'];
      } else {
        $setData['bannerInfo'][$key]['link'] = $info['bannerLink'];
      }
      $setData['bannerInfo'][$key]['target'] = $info['bannerTarget'];
      $setData['bannerInfo'][$key]['badgeText'] = $info['bannerBadgeText'];
      $setData['bannerInfo'][$key]['mainTitle'] = $info['bannerMainTitle'];
      $setData['bannerInfo'][$key]['subTitle'] = $info['bannerSubtitle'];
      $setData['bannerInfo'][$key]['buttonTitle'] = $info['bannerButtonTitle'];
      $setData['bannerInfo'][$key]['foregroundColor'] = $info['bannerForegroundColor'];

      if (in_array($info['bannerForegroundColor'], ['white', '#FFFFFF'])) {
        $setData['brightTextBannerIndices'][] = $actualIndex;
      }
      $actualIndex++;
    ...
    if (in_array(1, $setData['brightTextBannerIndices'])) {
      $setData['brightTextBannerIndices'][] = count($setData['bannerInfo']) + 1;
    }

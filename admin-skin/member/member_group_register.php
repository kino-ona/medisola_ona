    ...
    <div class="form-inline">
        <table class="table table-cols">
            ...
            <tr>
                <th>회원등급코드</th>
                <td>
                    <input type="hidden" name="chkSelfCode" id="chkSelfCode" value="<?= gd_isset($data['selfCode']) ?>"/>
                    <label>
                        <input type="text" name="selfCode" class="form-control js-maxlength" maxlength="40"
                                value="<?= gd_isset($data['selfCode']) ?>"/>
                    </label>
                    <button type="button" class="btn btn-sm btn-gray" id="overlap_selfCode" style="margin-left:50px;">코드 중복확인</button>
                </td>
            </tr>
            <tr>
                <th>등급표시</th>
...
<script type="text/javascript">
    ...
    //210929 디자인위브 mh 회원코드 중복확인
    $(function(){
        $('#overlap_selfCode').click(function(e){
            e.preventDefault();
            
            var selfCode = $('#frm input[name="selfCode"]').val();
            var sno = $('#frm input[name="sno"]').val();
            ...
            if(selfCode){
                $.ajax({
                    ...
                    success : function (data) {
                        ...
                        $('input[name="chkSelfCode"]').val(selfCode);
                        if(data.code == 0){
                            member.dialog({message: data.message});
                        }else{
                ...
                member.dialog({message: '회원등급코드가 없습니다.'});
                return false;
            }

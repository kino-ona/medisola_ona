    ...
    <div class="table-title gd-help-manual">
        ...
        <table class="table table-cols">
            ...
            <tr>
                <th>필터 Tag</th>
                <td>
                    <input type="text" name="tags" value="<?=gd_isset($data['tags']); ?>"
                                           class="form-control width90p js-maxlength" maxlength="255"/>
                </td>
                <th>최대선택 갯수</th>
                <td>
                    <label title=""><input type="number" step="1" name="limitCnt" value="<?=gd_isset($data['limitCnt']); ?>"
                                           class="form-control width-2xs" min="0" /></label> (골라담기 10개당 최대 선택 갯수, 0은 무제한)
                </td>
            </tr>
            <tr>
                <th>프리미엄 배수</th>
                <td>
                    <label title=""><input type="number" step="1" name="premiumMultiplier" value="<?=gd_isset($data['premiumMultiplier'], 1); ?>"
                                           class="form-control width-2xs" min="1" /></label> (초과 수량당 추가 가격 배수, 기본값 1)
                </td>
                <th>설명 이미지</th>
                <td>
                    <div class="form-inline">
                        <input type="file" name="subImageNm[]" class="form-control  cla_image_filed"/>
                        <?php if ($data['subImageNm']) { ?>
                            <input type="hidden" name="subImageNm[]" value="<?= $data['subImageNm'] ?>">
                            <?=gd_html_add_goods_image($data['addGoodsNo'], $data['subImageNm'], $data['imagePath'], $data['imageStorage'], 30, $data['goodsNm'], '_blank'); ?>
                            <label class="checkbox-inline"><input type="checkbox" name="subImageDelFl" value="y">삭제</label>
                            <?= $data['subImageNm'] ?>
                        <?php } ?>
                    </div>
                </td>
            ...
            <tr>
                <th>상품 요약 설명</th>
                <td colspan="3">
                    <input type="text" name="goodsSummary" value="<?=gd_isset($data['goodsSummary']); ?>"
                                           class="form-control width90p js-maxlength" maxlength="255"/>
                </td>
            </tr>
            <tr>

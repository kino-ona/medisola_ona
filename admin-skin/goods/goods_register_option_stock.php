...
<script type="text/javascript" xmlns="http://www.w3.org/1999/html">
    ...
    $(document).ready(function () {
        ...
        /**
         * 상품 이미지 파일 업로드 시 자동 압축 처리
         */
        $(document).on('change', 'input[type="file"]', function() {
            // name 속성이 image[...] 또는 imageFb[...] 형식인지 확인
            var name = $(this).attr('name') || '';
            if (!name.match(/^(image|imageFb)\[/)) {
                return;
            }
            var $fileInput = $(this);
            var file = this.files && this.files[0];
            
            console.log('window.CustomImageCompress', window.CustomImageCompress);
            if (!file) {
                return;
            }

            // 이미지 파일인지 확인
            if (window.CustomImageCompress && window.CustomImageCompress.isImageFile(file)) {
                // 이미지 압축 처리
                window.CustomImageCompress.compress(file, {
                    ...
                    // Preview 영역 업데이트 (있는 경우)
                    var $previewSpan = $fileInput.siblings('span[id*="PreView"]');
                    if ($previewSpan.length > 0) {
                        ...
                        reader.onload = function(e) {
                            ...
                            img.onload = function() {
                                var previewHtml = '<img src="' + e.target.result + '" style="max-width: 25px; max-height: 25px; margin: 5px;" />';
                                previewHtml += ' : 가로 ' + this.width + 'pixel(픽셀)';
                                $previewSpan.html(previewHtml);
                            };
                        };
    ...
    (function() {
        ...
        /**
         * 목표 용량에 맞게 품질 조정하여 압축 (JPEG/PNG 지원)
        ...
        function compressToTargetSize(canvas, mimeType, quality, targetSize) {
            return new Promise(function(resolve, reject) {
                ...
                // JPEG가 아닌 경우 원본 포맷 유지하며 리사이징만 수행
                if (!isJpeg) {
         ...
         * @param {number} options.quality - 품질 (0-1, JPEG만 적용)
         * @param {number} options.targetSize - 목표 용량 (bytes)
        ...
        function compressImage(file, options) {
            return new Promise(function(resolve, reject) {
                ...
                var quality = options.quality || 0.80; // JPEG 기본 품질
                ...
                // PNG인 경우 JPG로 변환
                var isPng = /^image\/png$/i.test(originalMimeType);
                var mimeType, fileName;
                if (isPng) {
                    mimeType = 'image/jpeg';
                    // 파일명 확장자를 .jpg로 변경
                    if (originalFileName.lastIndexOf('.') > 0) {
                        fileName = originalFileName.substring(0, originalFileName.lastIndexOf('.')) + '.jpg';
                    } else {
                ...
                shouldCompress(file).then(function(needsCompress) {
                    ...
                    img.onload = function() {
                        try {
                            ...
                            // 목표 용량에 맞게 압축 (PNG인 경우 JPG로 변환하여 압축)

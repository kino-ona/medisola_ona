...
<form id="frmList" action="" method="get" target="ifrmProcess">
    ...
    <div class="table-header">
        <div class="pull-left">
            ...
            <!-- 검색된 데이터 수와 전체 데이터 수 비교 : 전체 / 검색 해제 여부 -->
            <input type="hidden" name="searchCnt" value="<?= $page->recode['total'] == $page->recode['amount'] ?>">
        </div>
    ...
    <div class="form-inline">
        <table class="table table-rows">
            ...
            if (gd_isset($data)) {
                ...
                $searchSleepNo = array_column($sleepNoData, 'sleepNo');
                foreach ($data as $val) {
...
<script type="text/javascript">
    ...
    var msg = {
        ...
        DEL_ALL_MEM: "전체 <?= $page->recode['total'] > LIMIT_SLEEP_MEMBER_TO_PROCESS ? LIMIT_SLEEP_MEMBER_TO_PROCESS : $page->recode['total']; ?>명의 휴면회원을 탈퇴처리하시겠습니까?<br/>해당 회원은 즉시 탈퇴 처리되며, 탈퇴완료 시 취소할 수 없습니다.",
        ...
        BootstrapDialog.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ...
                    data.push({name: "searchSleepNo", value: "<?= json_encode($searchSleepNo) ?>"});
                    post_with_reload('../member/member_sleep_ps.php', data);
                }
            }
        });

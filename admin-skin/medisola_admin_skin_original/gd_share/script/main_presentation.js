...
$(document).ready(function () {
    ...
    $('#presentationMainTab > li').on('click', function (e) {
        ...
        if($('#presentationMainTab').data('period') == 1){
            if ($(this).data('name') == 'visit') {
                var periodIconSrcOn = periodIconSrc + '/ico_yesterday_on.png';
            } else {
                $.each($('#presentationMainTab > li'), function(key, target) {
                    if ($(target).data('name') == 'visit') {
                        $(target).find('.js-period-icon').attr('src', periodIconSrc + '/ico_yesterday_off.png');
                    }
                });
            }
        }

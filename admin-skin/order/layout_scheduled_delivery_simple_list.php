 ...
 * @modifier Conan Kim <kmakugo@gmail.com>
 */
...
<div class="table-responsive">
    <table class="table table-rows order-list">
        ...
        if (empty($data) === false && is_array($data)) {
            ...
            foreach ($data as $deliveryOrderNo => $orderData) {
                $rowCnt = $orderData['cnt']['goods']['all']; // 한 주문당 상품주문 수량
                $rowChk = 0; // 한 주문당 첫번째 주문 체크용
                $rowAddChk = 0; //
                $totalCnt++; // 주문서 수량
                $scheduledDeliverySno = null; // 회차배송 sno
                ...
                foreach ($orderData['goods'] as $sKey => $sVal) {
                    ...
                    foreach ($sVal as $dKey => $dVal) {
                        ...
                        foreach ($dVal as $key => $val) {
                            ...
                            if (is_null($scheduledDeliverySno)) {
                                $scheduledDeliverySno = $val['scheduledDeliverySno'];
                            }

                            $val['orderCellPhone'] =  StringUtils::numberToCellPhone($val['orderCellPhone']);

                            $deliveryStatusMode = substr($val['deliveryStatus'], 0, 1);
                            ...
                            // if (in_array($val['deliveryStatusMode'], $statusListCombine)) {
                            //     $checkBoxCd = $scheduledDeliverySno;
                            // } else {
                            //     $checkBoxCd = $deliveryOrderNo . INT_DIVISION . $val['sno'];
                            // }
                            // 회차배송은 회차배송 회차별로 체크하기 때문에 예외 없이 scheduledDeliverySno로 설정
                            $checkBoxCd = $scheduledDeliverySno;
                                    ...
                                    foreach($orderGridConfigList as $gridKey => $gridName){
                                        ...
                                        <?php if($gridKey === 'check'){ // 선택 ?>
                                            <?php if ($rowChk === 0) { ?>
                                                <td <?= $orderGoodsRowSpan; ?>>
                                                    <input type="checkbox" name="statusCheck[<?= $val['deliveryStatusMode'] ?>][]" <?= $checkDisabled ?> value="<?= $checkBoxCd; ?>"/>
                                                    <input type="hidden" name="orderStatus[<?= $val['deliveryStatusMode'] ?>][]" value="<?= $val['deliveryStatus']; ?>"/>
                                                    <input type="hidden" name="escrowCheck[<?= $val['deliveryStatusMode'] ?>][]" <?= $checkDisabled ?> value="<?= $val['escrowFl'] . $val['escrowDeliveryFl']; ?>"/>
                                                    <input type="hidden" name="orderChannelFl[<?= $val['deliveryStatusMode'] ?>][<?=$checkBoxCd?>]" value="<?= $val['orderChannelFl']; ?>"/>
                                                    <input type="hidden" name="deliveryMethodFl[<?= $val['deliveryStatusMode']; ?>][<?=$checkBoxCd?>]" value="<?= $val['deliveryMethodFl']; ?>"/>
                                                    <?php if (in_array($currentStatusCode, ['r', 'e', 'b'])) { ?>
                                                        <input type="hidden" name="handleSno[<?= $val['deliveryStatusMode'] ?>][]" value="<?= $val['handleSno']; ?>"/>
                                                        <input type="hidden" name="beforeStatus[<?= $val['deliveryStatusMode'] ?>][]" value="<?= $val['beforeStatus']; ?>"/>
                                                    <?php } ?>
                                        ...
                                        <?php if($gridKey === 'regDt' && $rowChk === 0){ // 주문일시 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="font-date nowrap">
                                                <?= str_replace(' ', '<br>', gd_date_format('Y-m-d H:i', $val[$gridKey])); ?>
                                            </td>
                                        <?php } // 주문일시 ?>

                                        <?php if($gridKey === 'estimatedDeliveryDt' && $rowChk === 0){ // 배송희망일 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="font-date nowrap">
                                                <?= str_replace(' ', '<br>', gd_date_format('Y-m-d', $val[$gridKey])); ?>
                                            </td>
                                        <?php } // 배송희망일 ?>

                                        <?php if($gridKey === 'round' && $rowChk === 0){ // 회차 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="nowrap">
                                                <?= number_format($val[$gridKey]); ?>
                                            </td>
                                        <?php } // 회차 ?>

                                        <?php if($gridKey === 'totalRound' && $rowChk === 0){ // 총 회차 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="nowrap">
                                                <?= number_format($val[$gridKey]); ?>
                                            </td>
                                        <?php } // 총 회차 ?>
                                        ...
                                        <?php if($gridKey === 'orderNo' && $rowChk === 0){ // 주문번호 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="order-no">
                                                ...
                                                <?php } ?>
                                                <a href="#;" onclick="javascript:open_order_link('<?=$val[$gridKey]?>', '<?=$openType?>', '<?=$isProvider?>')" title="주문번호" class="font-num<?=$isUserHandle ? ' js-link-order' : ''?>" data-order-no="<?=$val[$gridKey]?>" data-is-provider="<?= $isProvider ? 'true' : 'false' ?>"><?= $deliveryOrderNo; ?></a><img src="<?=PATH_ADMIN_GD_SHARE?>img/icon_grid_open.png" alt="팝업창열기" class="hand mgl5" border="0" onclick="javascript:order_view_popup('<?=$val[$gridKey]?>', '<?=$isProvider?>');" />
                                                <?php if ($val['orderChannelFl'] == 'naverpay') { ?>
                                                    <p>
                                                        <a href="#;" onclick="javascript:open_order_link('<?=$val[$gridKey]?>', '<?=$openType?>', '<?=$isProvider?>')" title="주문번호" class="font-num<?=$isUserHandle ? ' js-link-order' : ''?>" data-order-no="<?=$val[$gridKey]?>" data-is-provider="<?= $isProvider ? 'true' : 'false' ?>"><img
                                                ...
                                                <?php } else if ($val['orderChannelFl'] == 'etc') { ?>
                                                    <p>
                                                        <a href="#;" onclick="javascript:open_order_link('<?=$val[$gridKey]?>', '<?=$openType?>', '<?=$isProvider?>')" title="주문번호" class="font-num<?=$isUserHandle ? ' js-link-order' : ''?>" data-order-no="<?=$val[$gridKey]?>" data-is-provider="<?= $isProvider ? 'true' : 'false' ?>">
                                        ...
                                        <?php if($gridKey === 'goodsNo' ){ // 상품번호 ?>
                                            <td class="nowrap">
                                                <?= $val[$gridKey] ?>
                                            </td>
                                        <?php } // 상품번호 ?>

                                        <?php if($gridKey === 'goodsCd' ){ // 자체상품코드 ?>
                                            <td class="nowrap">
                                                <?= $val[$gridKey] ?>
                                            </td>
                                        <?php } // 자체상품코드 ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-left" style="border: 0px;">
                                                            ...
                                                            if (empty($val['optionInfo']) === false) {
																...
																// 웹앤모바일 2023-08-22 첫 배송일 추가
																if($val['firstDelivery']){
																	echo ', 첫 배송일 : '.$val['firstDelivery'];
																}
																...
																// 웹앤모바일 2023-08-22 첫 배송일 추가
																if($val['firstDelivery']){
																	echo '<div class="option_info">첫 배송일 : '.$val['firstDelivery'].'</div>';
																}
                                        ...
                                        <?php if($gridKey === 'goodsCnt'){ // 수량 ?>
                                            <td class="goods_cnt"><?= number_format($val[$gridKey]); ?></td>
                                        <?php } // 수량 ?>
                                        ...
                                        <?php if($gridKey === 'adminMemo' && $rowChk === 0){ // 관리자메모 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="text-center" data-order-no="<?= $val['orderNo'] ?>" data-reg-date="<?= $val['regDt'] ?>">
                                                <button type="button" class="btn btn-sm btn-<?php if($data[$deliveryOrderNo]['adminOrdGoodsMemo']){ echo 'gray'; }else{ echo 'white';} ?> js-super-admin-memo" data-order-no="<?= $val['orderNo']; ?>" data-memo="<?=$data[$deliveryOrderNo]['adminOrdGoodsMemo'];?>">보기</button>
                                            </td>
                                        <?php } // 관리자메모 ?>

                                        <?php if($gridKey === 'invoiceNo' && $rowChk === 0){ // 송장번호 ?>
                                                ...
                                                <?php if ($rowChk === 0) { ?>
                                                    <td <?= $orderGoodsRowSpan; ?>>
                                                        <?php if ($orderData['deliveryCombineDisplayMessage'] === true) { ?>
                                                            ...
                                                            <input type="hidden" name="invoiceIndividualUnsetFl[<?=$scheduledDeliverySno?>]" value="" />
                                                            <div class="js-invoice-unset-area" style="color: red;">
                                                                <input type="checkbox" name="invoiceIndividualUnset[]" value="<?=$scheduledDeliverySno?>" data-combine-prevent="<?=$orderData['deliveryCombinePrevent']?>"/> 개별등록 해제
                                                            </div>

                                                            <?php if($orderData['deliveryCombinePrevent'] === false) {  ?>
                                                                <div class="js-invoice-area display-none">
                                                                    <?= gd_select_box(null, 'invoiceCompanySno[' . $scheduledDeliverySno . ']', $deliveryCom, null, $selectInvoiceCompanySno, null); ?>
                                                                    <input type="text" name="invoiceNo[<?= $scheduledDeliverySno ?>]" value="" class="form-control input-sm mgt5"/>
                                                                </div>
                                                            <?php } ?>
                                                            <!-- 개별등록 해제 문구 노출  -->
                                                        ...
                                                        <?php } else { ?>
                                                            <?= gd_select_box(null, 'invoiceCompanySno[' . $scheduledDeliverySno . ']', $deliveryCom, null, $selectInvoiceCompanySno, null); ?>
                                                            <input type="hidden" name="invoiceIndividualUnsetFl[<?=$scheduledDeliverySno?>]" value="<?=$scheduledDeliverySno?>" />
                                                            <input type="text" name="invoiceNo[<?= $scheduledDeliverySno ?>]" value="<?= $val['invoiceNo']; ?>" class="form-control input-sm mgt5"/>
                                        ...
                                        <?php if($gridKey === 'deliveryDt' && $rowChk === 0){ // 배송희망일 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="font-date nowrap">
                                                <?= str_replace(' ', '<br>', gd_date_format('Y-m-d', $val[$gridKey])); ?>
                                            </td>
                                        <?php } // 배송희망일 ?>

                                        <?php if($gridKey === 'deliveryStatus' && $rowChk === 0){ // 배송희망일 ?>
                                            <td <?= $orderGoodsRowSpan; ?> class="font-date nowrap">
                                                <?php switch($val[$gridKey]) {
                                                        case 'p1':
                                                            echo '배송대기';
                                                            break;
                                                        case 'g1':

...
unset($arrQueryString['statusMode']);
...
<form id="frmSearchOrder" method="get" class="js-form-enter-submit">
    ...
    <div class="table-title <?=isset($currentUserHandleMode) ? '' : 'gd-help-manual'?>">
        회차 배송 검색
    <span class="search">
        <button type="button" class="btn btn-sm btn-black" onclick="set_search_config(this.form, '<?=$isOrderSearchMultiGrid?>')">검색설정저장</button></span>
        <span class="search mgr5">
            <button type="button" class="btn btn-sm btn-white" onclick="set_is_order_search_multi_grid('<?=$isOrderSearchMultiGrid == 'y' ? 'n' : 'y'; ?>')">
                <img src="/admin/gd_share/img/ico_transfer.png" style="margin-right:3px;"/>검색조건 변환
            </button>
        </span>
    </div>
    ...
    <div class="search-detail-box">
        <table class="table table-cols">
            ...
            <tr>
                ...
                <td>
                <label class="radio-inline">
                        <input type="radio" name="fdi" value="1" <?= gd_isset($checked['fdi']['1']); ?>/>포함
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="fdi" value="0" <?= gd_isset($checked['fdi']['0']); ?>/>미포함
                    </label>
                </td>
                <th>공급사 구분</th>
                <td>
            ...
            <tr>
                ...
                <td colspan="3">
                    <div class="form-inline">
                        ...
                        <div class="btn-group js-eta-dateperiod" data-toggle="buttons" data-target-name="treatDate[]">
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="0">오늘</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="-1">내일</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="-2">3일 후</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="-3">4일 후</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="-4">5일 후</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="-9">10일 후</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="-29">1개월 후</label> 
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="1">어제</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="6">7일 전</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="14">15일 전</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="29">1개월 전</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="89">3개월 전</label>
                            <label class="btn btn-white btn-sm hand "><input type="radio" name="searchPeriod" value="364">1년 전</label> 
                        </div>
                    </div>
                </td>
            </tr>
                ...
                <td colspan="3">
                    <div class="dl-horizontal dl-checkbox">
                        ...
                        foreach ($settle as $key => $val) {
                            ...
                            if ($key == 'pk' || $key == 'pn') { // 카카오페이 || 네이버페이 결제형
                                $payMethod = ' (간편결제)';
                            }
...
<!--                            </label>-->
    ...
    <ul class="nav nav-tabs mgb0" role="tablist">
        <?php foreach ($tabs as $key => $val) { 
            ?>
            <li role="presentation" <?=$search['statusMode'] == $key ? 'class="active"' : ''?>>
                <a href="../order/<?=$page->page['url']?>?statusMode=<?=$key?>&<?=$queryString ? 'searchFl=y&' . $queryString : ''?>"><?=$val?></a>
            </li>
        <?php } ?>
    </ul>
    ...
    <input type="hidden" name="statusMode" value="<?=$search['statusMode']?>"/>

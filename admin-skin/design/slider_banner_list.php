<?php
/**
 * 배너 이미지 HTML 태그 생성 함수 (webp 지원을 위해 기존 gd_html_banner_image 함수 대체)
 * 
 * @param string $imagePath 이미지 경로
 * @param string $alt alt 속성 값
 * @param int|string $width 원하는 너비 (픽셀 또는 빈 문자열)
 * @return string HTML img 태그
 */
function custom_gd_html_banner_image($imagePath, $alt = '', $width = '') {
    if (empty($imagePath)) {
        return '';
    }
    
    // webp인 경우에만 커스터마이징 로직 진행
    $fullPath = '/data/skin/' . $imagePath;
    
    $imgTag = '<img src="' . htmlspecialchars($fullPath) . '"';
        ...
        if (gd_isset($data)) {
            ...
            foreach ($data as $key => $val) {
                ...
                <tr class="text-center">
                    ...
                    <td><?php if (empty($bannerImage) === false) { echo custom_gd_html_banner_image($val['bannerImagePath'] . $bannerImage, $bannerImageAlt, 200); } ?></td>

                            ...
                            <tr class="banner-info">
                                ...
                                <td class="form-inline">
                                    ...
                                    <div class="mgt3 mgl15">
                                        <span class="display-inline-block" style="width:75px;">뱃지 문구</span>
                                        <input type="text" name="bannerBadgeText[]" value="<?php echo $iVal['bannerBadgeText']; ?>" maxlength="20" class="form-control js-maxlength width-xl"/>
                                    </div>
                                    <div class="mgt3 mgl15">
                                        <span class="display-inline-block" style="width:75px;">제목</span>
                                        <input type="text" name="bannerMainTitle[]" value="<?php echo $iVal['bannerMainTitle']; ?>" maxlength="100" class="form-control js-maxlength width-xl"/>
                                    </div>
                                    <div class="mgt3 mgl15">
                                        <span class="display-inline-block" style="width:75px;">부제목</span>
                                        <input type="text" name="bannerSubtitle[]" value="<?php echo $iVal['bannerSubtitle']; ?>" maxlength="100" class="form-control js-maxlength width-xl"/>
                                    </div>
                                    <div class="mgt3 mgl15">
                                        <span class="display-inline-block" style="width:75px;">버튼 문구</span>
                                        <input type="text" name="bannerButtonTitle[]" value="<?php echo $iVal['bannerButtonTitle']; ?>" maxlength="20" class="form-control js-maxlength width-xl"/>
                                    </div>
                                    <div class="mgt3 mgl15">
                                        <span class="display-inline-block" style="width:75px;">텍스트 색상</span>
                                        <input type="text" name="bannerForegroundColor[]" value="<?php echo $iVal['bannerForegroundColor']; ?>" maxlength="7" class="form-control js-maxlength width-xl"/>
                                    </div>
                                    <div class="mgt3 mgl15">
...
<script type="text/javascript">
    ...
    (function() {
        ...
        function shouldCompress(file) {
            return new Promise(function(resolve) {
                ...
                var needsCompress = file.size >= sizeThreshold;
        ...
        function compressToTargetSize(canvas, mimeType, quality, targetSize) {
            return new Promise(function(resolve, reject) {
                ...
                // JPEG/WebP인 경우 품질 조정으로 목표 용량 달성 시도
                var minQuality = 0.1;
         ...
         * @param {boolean} options.useWebP - WebP 변환 사용 여부 (기본값: false)
                ...
                shouldCompress(file).then(function(needsCompress) {
                    ...
                    img.onload = function() {
                        try {
                            ...
                            // WebP 변환하지 않고 JPEG가 아닌 경우 원본 반환
                            if (!useWebP) {
    ...
    /**
     * 배너 이미지 preview 처리 함수
     * @param {File|Blob} file - preview할 파일
     * @param {string} bannerName - 배너 이름 ('bannerImageFile[]', 'bannerNavActiveImageFile[]', 'bannerNavInactiveImageFile[]')
     * @param {number} idx - 인덱스
     */
    function processBannerPreview(file, bannerName, idx) {
        ...
        reader.onload = function (e) {
            if (bannerName == 'bannerImageFile[]') {
                ...
                image.onload = function() {
                    var bannerNavName = (bannerName == 'bannerNavActiveImageFile[]') ? 'bannerNavActive' : 'bannerNavInactive';
    ...
    $(document).ready(function () {
        ...
        $(document).on('change blur', 'select[name^=\'bannerSliderConf\'], input[name^=\'bannerSliderConf\'], input[name^=\'sideButton\'], input[name^=\'pageButton\'], input[name^=\'bannerSize\'], input[name^=\'bannerImageFile\'], input[name^=\'bannerNav\'], select[name=\'bannerSize[sizeType]\']', function () {
            ...
            if (bannerType == 'file') {
                ...
                if (file) {
                    // 이미지 압축 처리
                    if (window.CustomImageCompress?.isImageFile(file)) {
                        // 압축 중 표시 (선택사항)
                        var originalFileName = file.name;
                        
                        window.CustomImageCompress?.compress(file, {
                            ...
                            // 압축된 파일로 preview 처리
                            processBannerPreview(compressedFile, bannerName, idx);
                        }).catch(function(error) {
                            console.error('이미지 압축 실패:', error);
                            // 압축 실패 시 원본 파일로 처리
                            processBannerPreview(file, bannerName, idx);
                        });
                    } else {
                        // 이미지가 아니거나 압축 모듈이 없으면 기존 로직 실행
                        processBannerPreview(file, bannerName, idx);
                    }
                }
            }
        ...
        function addBanner(len) {
            ...
            $target.find('input[name="bannerBadgeText[]"]').val('');
            $target.find('input[name="bannerMainTitle[]"]').val('');
            $target.find('input[name="bannerSubtitle[]"]').val('');
            $target.find('input[name="bannerButtonTitle[]"]').val('');
            $target.find('input[name="bannerForegroundColor[]"]').val('');

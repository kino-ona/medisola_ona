...
class GoodsAdmin extends \Component\Goods\Goods
{
    ...
    public function getDataGoods($goodsNo = null, $taxConf, $applyFl = false)
    {
        ...
        */
        ...
        $data['naverShoppingMobileRentalPay'] = is_numeric($naverGoods['shoppingMobileRentalPay']) ? $naverGoods['shoppingMobileRentalPay']: 0;
    ...
    public function saveInfoGoods($arrData)
    {
        ...
        */
        ...
        if(count($arrData['naverNpayAble']) == 2) $arrData['naverNpayAble'] = 'all';
        else if(count($arrData['naverNpayAble']) == 0) $arrData['naverNpayAble'] = 'no';
        else $arrData['naverNpayAble'] = $arrData['naverNpayAble'][0];

        if(count($arrData['naverNpayAcumAble']) == 2) $arrData['naverNpayAcumAble'] = 'all';
        else if(count($arrData['naverNpayAcumAble']) == 0) $arrData['naverNpayAcumAble'] = 'no';
        ...
        if ($arrData['naverProductFlag'] == 'r') {
            $goodsNaver = [
                ...
                'shoppingMobileRentalPay' => $arrData['naverShoppingMobileRentalPay'],
                    ...
                    foreach ($arrFileData['name'][$val] as $fKey => $fVal) {
                        foreach ($fVal as $vKey => $vVal) {
                            ...
                            if (gd_file_uploadable($arrFileData, 'image', $val, $fKey, $vKey) === true) {
                                ...
                                $newImageName = base64_encode($tmpName).$imageExt; // 이미지 공백 제거
    ...
    public function imageUploadGoods($arrFileData, $strImageStorage, $arrImageSize, $arrImageResize, $imageInfo,$goodsNo, $mode = null, &$goodsImageMigrationLog = [], array $extraArrData = [])
    {
        ...
        if (!in_array($strImageStorage, ['url', 'local']) && !empty($extraArrData['originalGoodsNo'])) {
            $originalGoodsImageInfoArr = $this->getGoodsImage($extraArrData['originalGoodsNo']) ?: [];
            foreach (array_column($originalGoodsImageInfoArr, 'imageName') as $originalImageName) {
                if (in_array($originalImageName, $tmpInfo['imageName']) === true) {
                    continue;
                }
    ...
    public function setCopyGoods($goodsNo)
    {
        ...
        foreach ($arrGoodsTable as $goodsTableNm) {
            ...
            if ($functionNm == 'TableGoods') {
                $fieldData = DBTableField::setTableField($functionNm, null, array('goodsNo', 'regDt', 'modDt', 'orderCnt', 'orderGoodsCnt', 'hitCnt', 'cartCnt', 'wishCnt', 'reviewCnt', 'plusReviewCnt', 'naverReviewCnt'));
                //$addField = ',imagePath';
    ...
    public function setDeleteGoods($goodsNo)
    {
        ...
        // --- 이미지 삭제 처리
        if (!in_array($data['imageStorage'], ['obs', 'url']) && $data['imagePath']) {
    ...
    protected function setAdminListGoods(&$getData,$addField = null,$mode = null)
    {
        ...
        $strField = "g.goodsNo,g.applyFl,g.goodsNm, g.imageStorage, g.imagePath,g.goodsPrice,g.goodsDisplayFl,g.goodsDisplayMobileFl,g.goodsSellFl,g.goodsSellMobileFl,g.totalStock,g.stockFl,g.soldOutFl,g.regDt,g.modDt,g.delDt,g.applyType,g.applyMsg,g.applyDt,g.scmNo,g.purchaseNo,g.goodsDescription,g.goodsDescriptionMobile" . $addField;
        $strSQL = 'SELECT ' . $strField . ' FROM ' . DB_GOODS . ' g ' . $join . ' WHERE g.goodsNo IN ("'.implode('","',array_column($getData, 'goodsNo')).'")';
        ...
        $strNaverBookSQL = 'SELECT goodsNo, naverbookFlag FROM ' . DB_NAVER_BOOK . ' g  WHERE goodsNo IN ('.implode(',',array_column($getData, 'goodsNo')).')';
        $tmpNaverBookData = $this->db->query_fetch($strNaverBookSQL);
        $naverBookData = array_combine (array_column($tmpNaverBookData, 'goodsNo'), array_column($tmpNaverBookData, 'naverbookFlag'));
    ...
    public function getAdminListBatch($mode = null)
    {
        ...
        switch ($mode) {
            ...
            case 'delivery':
                ...
                $addField = ",g.deliverySno, ". implode(', ', $arrField);
                break;
            case 'restock':
                if (gd_isset($getValue['pagelink'])) {
                    $getValue['page'] = (int)str_replace('page=', '', preg_replace('/^{page=[0-9]+}/', '', gd_isset($getValue['pagelink'])));
    ...
    */
    ...
    public function saveInfoPopupCategory($arrData)
    {
        ...
        if($arrData['mode'] == 'popup_category_change') {
            foreach ($arrData['goodsNo'] as $key => $val) {
                ...
                $this->db->set_update_db(DB_GOODS_SEARCH, $arrBind['param'], 'goodsNo = "' . $arrData['goodsNo'][$key] . '"', $arrBind['bind']);
                unset($arrBind);
            }
        }else{
            foreach ($arrData['goodsNo'] as $key => $val) {
    ...
    public function saveInfoDeliverySchedule($arrData,$goodsNo) {
        ...
        if ($getDeliverySchedule === false) { // 등록
            ...
            if ($updateFl) { // 수정
                $arrBind = $this->db->get_binding(DBTableField::getBindField('tableGoodsDeliverySchedule', array_keys($deliveryScheduleData)), $deliveryScheduleData, 'update');
                $this->db->bind_param_push($arrBind['bind'], 'i', $goodsNo);

...
class OrderNew
{
    ...
    public function generateOrderNo()
    {
        ...
        try {
            $result = $this->db->query('INSERT INTO es_orderNoSequence VALUES ()');
        } catch (\Exception $e) {
            throw new Exception($e->getMessage());
        }
        $tmpSequence = sprintf('%06d', $this->db->insert_id());

        // PREFIX_ORDER_NO (년월일시분초) 에 마이크로초 정수화 한 값을 붙여 주문번호로 사용함, 16자리 주문번호임
        return PREFIX_ORDER_NEW_NO . $tmpSequence;
    }
    ...
    public function saveOrder($orderInfo, $order, $memberData, $deliveryInfo, $aCartData, $history, $couponInfo, $taxPrice)
    {
        ...
        if ($orderInfo['settleKind'] == 'gb') {
            ...
            $this->statusChangeCodeP($this->orderNo, $arrOrderGoodsSno);
        } else {
    ...
    public function saveOrderInfo($cartInfo, $orderInfo, $order, $checkSumData = true)
    {
        ...
        if (Request::isByapps() && Request::isMobileDevice()) {
            ...
            if (empty($matches) === false) {
                $order['appOs'] = (in_array(current($matches), ['iPhone','iPad','iPod'])) ? 'ios' : 'android';
            } else {
        ...
        // 주문로그 저장
        \Logger::channel('order')->info(__METHOD__ . 'OREDR NO : ' . $this->orderNo, $orderInfo);
    ...
    public function statusChangeCodeP($orderNo, $arrData, $frontFlag = false)
    {
        ...
        if ($frontFlag == false) {
            ...
            if ($orderFunctionProcess === true) {
                ...
                // sms발송안하고 발송하라고 flag값만 리턴
                return array('smsFl' => 'T');
            } else {
                return array('smsFl' => 'F');
            }
        }
    }
                ...
                if (empty($optionData) === false) {
                    if ($optionData['stockFl'] == 'y') {
                        ...
                        if ($optionData['stockCnt'] > 0 && $optionData['stockCnt'] - $val['goodsCnt'] >= 0) {
                            ...
                            // 상품 재고 수정
                            $updateResult = $this->db->set_update_db(DB_GOODS_OPTION, 'stockCnt = stockCnt - ' . $val['goodsCnt'], str_replace('go.', '', implode(' AND ', $arrWhere)), $arrBind);

...
class Cart
{
    ...
    protected function setInsetCart($arrData)
    {
        ...
        if ($arrData['directCart'] != 'y' && $this->isWrite !== true) {
    ...
    public function checkOrderPossible($data, $whsiFl = false)
    {
        ...
        switch ($data['goodsPermission']) {
            ...
            case 'group':
                ...
                if (!in_array($this->_memInfo['groupSno'], $group)) {
                    $data['orderPossible'] = 'n';
            ...
            if ($this->_memInfo['dc' . ucwords($this->_memInfo['dcType'])] > 0 || $this->memberDc['totalDcBrandBankPercent'] > 0) {
                ...
                if ($data['addDcFl'] === true && empty($this->_memInfo['dcExOption']) === false && in_array('category', $this->_memInfo['dcExOption']) === true && empty($this->_memInfo['dcExCategory']) === false) {
                    ...
                    foreach ($goodsCateCd as $key=>$val){
                        if (in_array($val, $this->_memInfo['dcExCategory']) === true) {
                            $data['addDcFl'] = false;
                        }
                    }
            ...
            if(in_array('coupon', $exceptBenefit) === false || ($dataVal['exceptBenefitGroup'] == 'group' && in_array($this->_memInfo['groupSno'], $exceptBenefitGroupInfo) === false)) {
                ...
                if ($memberCouponData['couponApplyProductType'] !== 'all') {
                    ...
                    if ($memberCouponData['couponApplyProductType'] == 'category'){
                        ...
                        $crossCategory = array_intersect($applyCategoryGroupArr, $cateCdArr);
                        if (!$crossCategory) {
                            continue;
                        }
                    }
                    ...
                    if ($memberCouponData['couponApplyProductType'] == 'brand') {
                        ...
                        if(in_array($dataVal['brandCd'], $applyBrandGroupArr) == false) {
                            continue;
                        }
                    }
                    // 상품
                    if ($memberCouponData['couponApplyProductType'] == 'goods') {
                        ...
                        if (in_array($dataVal['goodsNo'], $applyGoodsGroupArr) == false) {
                            continue;
                        }
                    }
                    // 공급사
                    if($memberCouponData['couponApplyProductType'] == 'provider') {
                        ...
                        if (in_array($dataVal['scmNo'], $applyProviderGroupArr) == false) {
                            continue;
                        }
                    }
                }

    ...
    public function index()
    {
        try {
            ...
            foreach($orderData as $key => $valArr){
                $orderData[$key]['orderGoodsCnt'] = count($valArr['goods']);
            }
            ...
            // 사은품 갯수
            $this->setData('totalGiftCnt', count($orderData['gift']));
            ...
            if (!Session::has(SESSION_GLOBAL_MALL) && (in_array(substr($orderData['orderStatus'], 0, 1), $order->statusStandardCode['d']) === true || substr($orderData['orderStatus'], 0, 1) == 'o')) {
                $receipt['particularFl'] ='y';
                if (gd_is_plus_shop(PLUSSHOP_CODE_RECEPTION) === true && in_array($orderData['settleKind'], ['gb', 'gz', 'gd', 'gm']) && substr($orderData['orderStatus'], 0, 1) != 'o') {
                    ...
                    if (is_file($pgModule) === true) {
                        ...
                        if ($methodExists === true) {
                            ...
                            if (empty($escrowScript) === false) {
                                $escrowScript = implode(PHP_EOL, $escrowScript);
                            }
                        }
                        unset($pg);
                    }

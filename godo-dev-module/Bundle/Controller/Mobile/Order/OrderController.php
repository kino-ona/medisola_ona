            ...
            if ($settle['general']['pn']['useFl'] == 'y') {
                ...
                foreach ($cartInfo as $cIval1) {
                    foreach ($cIval1 as $cIval2) {
                        foreach ($cIval2 as $cIKey3) {
                            if (in_array('goods', $naverPayConfig['presentExceptFl']) && in_array($cIKey3['goodsNo'], $naverPayConfig['exceptGoods'])) {
                                unset($settle['general']['pn']);
                            }
                            if (in_array('brand', $naverPayConfig['presentExceptFl']) && in_array($cIKey3['brandCd'], $naverPayConfig['exceptBrand'])) {
                                unset($settle['general']['pn']);
                            }
                            foreach ($cIKey3['cateAllCd'] as $cateVal1) {
                                if (in_array('category', $naverPayConfig['presentExceptFl']) && in_array($cateVal1['cateCd'], $naverPayConfig['exceptCategory'])) {
                                    unset($settle['general']['pn']);
                                }
                            }
                        }

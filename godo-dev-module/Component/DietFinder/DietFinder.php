...
class DietFinder
{
    ...
    /**
     * 영양 라인 계산
     * - 기존 4개 라인: 고단백/저나트륨/오메가3/300kcal이하 (기준치 조정)
     * - 신규 6개 라인: 저당/고식이섬유/저지방/저포화지방/저콜레스테롤/저탄수화물
    ...
    public function calculateLines($item)
    {
        ...
        $omega3 = floatval($item['nutrition_omega3'] ?? 0);
        ...
        if ($sodium > 0 && $sodium <= 450) $lines[] = '저나트륨';  // 400 → 450mg
        if ($omega3 >= 2) $lines[] = '오메가3';  // 2g (2000mg), DB 단위: g
        // 칼로리 라인: 550kcal(고칼로리)만 강조 라인으로 사용 (440kcal은 기본값이므로 제외)
        if ($calories > 500) $lines[] = '550kcal 라인';

        // 신규 라인 (Tier 1: 실제 데이터 기반)
    ...
    public function getRecommendedLinesByCondition($conditions, $goal = null)
    {
        ...
        $keyMap = [
            'lowSugar' => 'low-sugar', 'highFiber' => 'high-fiber', 'lowCarbs' => 'low-carbs',
            'lowSodium' => 'low-sodium', 'omega3' => 'omega3', 'lowSaturatedFat' => 'low-saturated-fat',
            'lowCholesterol' => 'low-cholesterol', 'highProtein' => 'high-protein',
            'lowFat' => 'low-fat', 'cal550' => 'cal-550',
        ];
        ...
        $lineDb = [
            ...
            'high-fiber'        => ['label' => '고식이섬유 라인', 'color' => '#84CC16', 'benefits' => ['장 건강', '혈당 조절', '콜레스테롤 감소']],
            'low-fat'           => ['label' => '저지방 라인', 'color' => '#8B5CF6', 'benefits' => ['심장 건강', '체지방 감소', '소화 개선']],
            ...
            'low-carbs'         => ['label' => '저탄수화물 라인', 'color' => '#F97316', 'benefits' => ['혈당 안정', '체지방 감소', '인슐린 감수성']],
        ];
        ...
        if (in_array('당뇨', $conditions)) {
            ...
            $recommendations[] = ['line' => 'highFiber', 'priority' => 2, 'reason' => '혈당 흡수 완화'];
            $recommendations[] = ['line' => 'lowCarbs', 'priority' => 3, 'reason' => '혈당 부하 감소'];
        }

        if (in_array('고혈압', $conditions)) {
            $recommendations[] = ['line' => 'lowSodium', 'priority' => 1, 'reason' => '혈압 관리'];
            $recommendations[] = ['line' => 'omega3', 'priority' => 2, 'reason' => '혈관 건강'];
            $recommendations[] = ['line' => 'lowSaturatedFat', 'priority' => 3, 'reason' => '혈관 건강'];
        }

        if (in_array('고지혈증', $conditions)) {
            $recommendations[] = ['line' => 'lowCholesterol', 'priority' => 1, 'reason' => '콜레스테롤 관리'];
            $recommendations[] = ['line' => 'lowSaturatedFat', 'priority' => 2, 'reason' => '포화지방 제한'];
            $recommendations[] = ['line' => 'omega3', 'priority' => 3, 'reason' => 'HDL 콜레스테롤 증가'];
        }

        if (in_array('심혈관질환', $conditions)) {
            $recommendations[] = ['line' => 'lowSaturatedFat', 'priority' => 1, 'reason' => '혈관 건강'];
            $recommendations[] = ['line' => 'lowCholesterol', 'priority' => 2, 'reason' => '혈관 건강'];
            $recommendations[] = ['line' => 'omega3', 'priority' => 3, 'reason' => '혈관 보호'];
        }
        ...
        if ($goal === '체중관리') {
            ...
            $recommendations[] = ['line' => 'lowFat', 'priority' => 2, 'reason' => '지방 제한'];
            $recommendations[] = ['line' => 'lowSugar', 'priority' => 3, 'reason' => '당류 제한'];
        }
        ...
        if ($goal === '혈당관리') {
            $recommendations[] = ['line' => 'lowSugar', 'priority' => 1, 'reason' => '혈당 안정화'];
            $recommendations[] = ['line' => 'highFiber', 'priority' => 2, 'reason' => '혈당 흡수 완화'];
            $recommendations[] = ['line' => 'lowCarbs', 'priority' => 3, 'reason' => '혈당 부하 감소'];
        }

        if ($goal === '심혈관건강') {
            $recommendations[] = ['line' => 'omega3', 'priority' => 1, 'reason' => '혈관 보호'];
            $recommendations[] = ['line' => 'lowSodium', 'priority' => 2, 'reason' => '혈압 관리'];
            $recommendations[] = ['line' => 'lowSaturatedFat', 'priority' => 3, 'reason' => '혈관 건강'];
        }

        if ($goal === '체중증가') {
            $recommendations[] = ['line' => 'highProtein', 'priority' => 1, 'reason' => '근육량 증가'];
            $recommendations[] = ['line' => 'cal550', 'priority' => 2, 'reason' => '충분한 칼로리'];
        }

        if ($goal === '임신수유부') {
            $recommendations[] = ['line' => 'highProtein', 'priority' => 1, 'reason' => '태아 발달'];
            $recommendations[] = ['line' => 'omega3', 'priority' => 2, 'reason' => '두뇌 발달'];
            $recommendations[] = ['line' => 'highFiber', 'priority' => 3, 'reason' => '변비 예방'];
        }

        if ($goal === '성장관리') {
            $recommendations[] = ['line' => 'highProtein', 'priority' => 1, 'reason' => '성장 지원'];
            $recommendations[] = ['line' => 'cal550', 'priority' => 2, 'reason' => '에너지 공급'];
        }

        if ($goal === '노화관리') {
            $recommendations[] = ['line' => 'highProtein', 'priority' => 1, 'reason' => '근감소증 예방'];
            $recommendations[] = ['line' => 'omega3', 'priority' => 2, 'reason' => '인지 기능 유지'];
            $recommendations[] = ['line' => 'highFiber', 'priority' => 3, 'reason' => '소화 건강'];
        }
    ...
    public function buildImageUrl($item)
    {
        ...
        // 2. imagePath + subImageNm 조합 (대체 이미지)
    ...
    public function getConditionsFromResponse($responseSno)
    {
        ...
        $recommendedLines = $response['recommended_lines'] ?? [];

        // 폴백: DB에 추천 라인이 비어있으면 통합 알고리즘으로 재생성
        // (레거시 알고리즘으로 저장된 기존 데이터 호환)
        if (empty($recommendedLines) && !empty($response['conditions'])) {
    ...
    public function generateNutritionReport($data)
    {
        ...
        $diseaseIds = ['diabetes', 'gestational-diabetes', 'kidney-pre-dialysis', 'kidney-dialysis', 'cancer'];
        $hasDisease = !empty(array_intersect($rawConditions, $diseaseIds));
        $pal = $hasDisease ? 1.2 : 1.55;
        $dailyCalories = max(1200, min(3000, round($bmr * $pal / 100) * 100)); // 100단위 반올림, 1200~3000 제한
        ...
        // elseif ($gender === 'male' && $age >= 40) $coachingMessages[] = '40대 이후 남성은 복부 지방 축적과 전립선 건강에 주의가 필요합니다. 식이섬유와 항산화 영양소 섭취를 늘리세요.';
        ...
        $condStrategyMap = [
            'weight-loss' => [
                ...
                ['id' => 'saturated-fat-down', 'reason' => '포화지방 섭취를 줄여 심혈관 건강을 유지하면서 체중 감량을 돕습니다.'],
            ],
            'general' => [
        ...
        $cta = [
            ...
            'subtext' => '예상 4주 후 변화: 체지방 감소 + 에너지 안정감 증가',
        ];
    ...
    public function saveQuizResponse($data)
    {
        ...
        $recommendedLinesJson = json_encode($recommendedLines, JSON_UNESCAPED_UNICODE);

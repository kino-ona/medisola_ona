...
class GoodsDisplayGroupReviewWidget extends \Widget\Mobile\Widget
{
    public function index()
    {
        try {
            ...
            $eventContents = $eventData['mobileContents'] ?? '';
            if (($eventData['descriptionSameFl'] ?? '') === 'y') {
                $eventContents = $eventData['pcContents'] ?? '';
            }
            ...
            foreach ($groupDataList as $idx => $group) {
                ...
                $groupTitleHtml = $this->buildGroupTitle($group, 'mobile');
            ...
            $this->setData('eventThemeContents', $eventContents);
            ...
            Logger::channel('crema')->error('Mobile GoodsDisplayGroupReviewWidget error', [

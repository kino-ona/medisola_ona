        ...
        switch (Request::request()->get('mode')) {
            ...
            case 'orderSelect':
                try {
                    ...
                    $returnUrl = '../gift_order/order.php?cartIdx=' . $cartIdx;
            ...
            case 'goodsCouponOrderApplyUseCheck':
                try {
                    if($postValue['memberCouponNo']) {
                        ...
                        $useAbleFlag = $coupon->getProductCouponGiftUsableCheck($postValue['memberCouponNo'], $postValue['cartSno'], $cartInfo, $cart->totalPrice);
                        echo $useAbleFlag;
                    }
                } catch (Exception $e) {
                    $this->json([
			...
			case 'CheckGiftCart' : 
				if(count($postValue)){
					$sno = $cart-> saveGiftInfo($postValue);
            ...
            case 'orderGift':
                try {
					$giftCartSno = $cart->getCartSno($postValue['cartSno']);
       
                    $cartIdx = $cart->setOrderSelect($giftCartSno);
                    $returnUrl = '../gift_order/order.php?cartIdx=' . $cartIdx;
                    throw new AlertRedirectException(null, null, null, $returnUrl, 'parent');
                } catch (Exception $e) {
                    throw $e;

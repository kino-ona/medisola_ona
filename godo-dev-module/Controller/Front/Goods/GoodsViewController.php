...
class GoodsViewController extends \Bundle\Controller\Front\Goods\GoodsViewController
{
    public function index()
    {
        ...
        if (isset($getValue['exportSubscriptionCsv']) && $getValue['exportSubscriptionCsv'] == '1') {
            $this->exportSubscriptionCsv();
            exit;
        }

        // CSV Export: 구독 스케줄 목록 (스케줄 기준 1행 + 신청 정보)
        if (isset($getValue['exportSubscriptionScheduleCsv']) && $getValue['exportSubscriptionScheduleCsv'] == '1') {

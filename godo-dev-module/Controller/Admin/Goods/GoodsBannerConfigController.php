...
class GoodsBannerConfigController extends \Controller\Admin\Controller
{
    public function index()
    {
        $this->callMenu('goods', 'goods', 'goodsBannerConfig');
        try {

            $in = \Request::request()->all();
            if($in['useGoodsViewBanner']){
                $this->setData('useGoodsViewBanner', $in['useGoodsViewBanner']);
            }else {
                $this->setData('useGoodsViewBanner', 'all');
            ...
            if ($getData && $getData['data']) {
                foreach ($getData['data'] as $key => $val) {
                    ...
                    $eData = $goodsBanner->getGoodsBanner($val['goodsNo']);
                    $getData['data'][$key]['goodsBanner'] = $eData;
                }
            }

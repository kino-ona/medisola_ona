...
class GoodsIconConfigController extends \Controller\Admin\Controller
{
    public function index()
    {
        $this->callMenu('goods', 'goods', 'goodsIconConfig');

        try {
            $in = \Request::request()->all();
            if($in['useGoodsViewIcon']){
                $this->setData('useGoodsViewIcon', $in['useGoodsViewIcon']);
            }else {
                $this->setData('useGoodsViewIcon', 'all');
            ...
            if ($getData && $getData['data']) {
                foreach ($getData['data'] as $key => $val) {
                    ...
                    $eData = $goodsIcon->getGoodsIcon($val['goodsNo']);
                    $getData['data'][$key]['goodsIcon'] = $eData;
                }
            }
